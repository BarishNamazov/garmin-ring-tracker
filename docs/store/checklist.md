# Upload the latest Ring Tracker release to Garmin

Use the signed package from the [latest GitHub release](https://github.com/BarishNamazov/garmin-ring-tracker/releases/latest) to update the existing Ring Tracker listing. Garmin submission and physical-watch verification remain manual.
...
| [listing.md](listing.md) | Description and What's New text; review it for the release being uploaded |

Download the files attached to the release. The device-specific `.prg` files are for USB sideloading, not Store upload.
...
3. Upload `RingTracker.iq` and enter the version shown on the release page, without the leading `v`. Wait for package validation.
...
The production application UUID remains `99f92e0c-a120-4641-b832-6da4d958585b`. The package uses the existing permanent signing key; check the release's compatibility list for supported watches. A separate beta listing would need its own application UUID; the production release is intended for the existing public listing.
